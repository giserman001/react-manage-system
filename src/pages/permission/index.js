...
import { Card, Button, Modal, Form, Radio, Tree, Select, Input, Transfer } from 'antd';
  ...
  getAuthUserList = (dataSource) => {
    ...
    if (dataSource && dataSource.length > 0) {
      for (let i = 0; i < dataSource.length; i++) {
        ...
        // 注意這裡寫法（targetKeys）还有这里的mockData是全部数据，不是互斥数据，组件会帮你过滤
        if (data.status == 1) {
...
class PermEditForm extends React.Component {
  state = {};
  // TODO 需要注意的点：：：：： 设置选中的节点，通过父组件方法再传递回来
  onCheck = (checkedKeys) => {
    // 调用父组件方法
